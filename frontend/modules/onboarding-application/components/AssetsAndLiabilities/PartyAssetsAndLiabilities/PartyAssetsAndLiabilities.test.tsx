...
describe('AssetsAndLiabilitiesWidget', () => {
    ...
    it('Should render AssetsAndLiabilitiesWidget', () => {
        ...
        expect(
            getByText(partyAssetsAndLiabilities.APPLICANT_TOTAL_ASSETS.replace('{{applicant}}', mockFinancialCategoriesApplicants[0].name))
        ).toBeInTheDocument();
        expect(
            getByText(partyAssetsAndLiabilities.APPLICANT_TOTAL_LIABILITIES.replace('{{applicant}}', mockFinancialCategoriesApplicants[0].name))
        ).toBeInTheDocument();
    });
