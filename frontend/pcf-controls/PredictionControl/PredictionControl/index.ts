...
import { PredictionContainer } from '@fsi/pcf-common/dist/containers/PredictionContainer/PredictionContainer';

export class PredictionControl implements StandardControl<IInputs, IOutputs> {
    ...
    public async init(context: IPropBag<IInputs>, notifyOutputChanged: () => void, state: ComponentFramework.Dictionary, container: HTMLDivElement) {
        init(context, 'MicrosoftPCF.PredictionControl');
        loadImages(context, ['emptyState48', 'error48', 'error100']);
    ...
    public updateView(context: IPropBag<IInputs>): void {
        updateView(this._container, PredictionContainer, { context });
    }
