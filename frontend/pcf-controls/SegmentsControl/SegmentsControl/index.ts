...
import { SegmentsContainer } from '@fsi/pcf-common/dist/containers/SegmentsContainer/SegmentsContainer';

export class SegmentsControl implements StandardControl<IInputs, IOutputs> {
    ...
    public async init(context: IPropBag<IInputs>, notifyOutputChanged: () => void, state: ComponentFramework.Dictionary, container: HTMLDivElement) {
        init(context, 'MicrosoftPCF.SegmentsControl');
    ...
    public updateView(context: IPropBag<IInputs>): void {
        updateView(this._container, SegmentsContainer, { context });
    }
